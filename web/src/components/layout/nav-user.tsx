...
export function NavUser() {
  ...
  const navTo = useCallback(
    (page: string) => (routePrefix ? `/${routePrefix}/${page}` : `/${page}`),
    [routePrefix],
  );
  ...
  const initials =
    displayName.split(" ").slice(-1)[0]?.slice(0, 2).toUpperCase() ?? "KH";
              ...
              <TooltipTrigger asChild>
                ...
                >
                  <Avatar className="h-8 w-8 rounded-lg grayscale shrink-0">
                    <AvatarImage
                      src={isAuthenticated ? "/avatars/technician.jpg" : ""}
                      alt={displayName}
                    />
            ...
            >
              ...
              {isAuthenticated ? (
                ...
                <>
                  <DropdownMenuGroup>
                    <DropdownMenuItem
                      onClick={() => navigate(navTo("settings"))}
                    >
