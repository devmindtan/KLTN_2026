...
export function ConfidenceCards({ predictionConfidence, errorConfidence }: ConfidenceCardsProps) {
  return (
    <div className="grid gap-3 md:grid-cols-2">
      <Card>
        ...
        <CardContent className="px-4 pb-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold tabular-nums">{(predictionConfidence.score * 100).toFixed(1)}%</span>
          </div>
          <Progress value={predictionConfidence.score * 100} className="h-2" />
          <Separator />
          <div className="grid grid-cols-3 gap-2 text-center">
            <div>
              <p className="text-xs font-semibold tabular-nums">{predictionConfidence.avg_input_samples}</p>
              <p className="text-[10px] text-muted-foreground">avg Input</p>
            </div>
            <div>
              <p className="text-xs font-semibold tabular-nums">{predictionConfidence.avg_lag_samples}</p>
              <p className="text-[10px] text-muted-foreground">avg LAG</p>
            </div>
            <div>
              <p className="text-xs font-semibold tabular-nums text-red-600 dark:text-red-400">{predictionConfidence.low_sample_count.toLocaleString("vi-VN")}</p>
      ...
      <Card>
        ...
        <CardContent className="px-4 pb-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-muted-foreground">Điểm tin cậy</span>
            <span className="text-2xl font-bold tabular-nums">{(errorConfidence.score * 100).toFixed(1)}%</span>
          </div>
          <Progress value={errorConfidence.score * 100} className="h-2" />
          <Separator />
          <div className="grid grid-cols-2 gap-2 text-center">
            <div>
              <p className="text-xs font-semibold tabular-nums">{errorConfidence.avg_sync_samples}</p>
              <p className="text-[10px] text-muted-foreground">avg Sync samples</p>
            </div>
            <div>
              <p className="text-xs font-semibold tabular-nums text-red-600 dark:text-red-400">{errorConfidence.mismatched_count.toLocaleString("vi-VN")}</p>
