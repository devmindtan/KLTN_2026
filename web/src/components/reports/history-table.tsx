...
> = {
  CREATE_REPORT: {
    label: "Tạo mới",
    icon: <IconPlus className="size-3.5 text-blue-500" />,
    badge: "text-blue-700 border-blue-200 bg-blue-50 dark:bg-blue-950/30",
  },
  DELETE_REPORT: {
    label: "Xóa",
    icon: <IconTrash className="size-3.5 text-red-500" />,
    badge: "text-red-700 border-red-200 bg-red-50 dark:bg-red-950/30",
  },
  DOWNLOAD_PDF: {
    label: "Tải PDF",
    icon: <IconDownload className="size-3.5 text-green-500" />,
    badge: "text-green-700 border-green-200 bg-green-50 dark:bg-green-950/30",
  },
  DOWNLOAD_XLSX: {
    label: "Tải XLSX",
    icon: <IconDownload className="size-3.5 text-green-500" />,
...
function getActionConfig(actionRaw: string) {
  const action = (actionRaw || "").trim();
  const actionUpper = action.toUpperCase();

  if (ACTION_CONFIG[action]) return ACTION_CONFIG[action];
  if (ACTION_CONFIG[actionUpper]) return ACTION_CONFIG[actionUpper];

  if (actionUpper.includes("CREATE")) return ACTION_CONFIG.CREATE_REPORT;
  if (actionUpper.includes("DELETE")) return ACTION_CONFIG.DELETE_REPORT;
  if (actionUpper.includes("DOWNLOAD") || actionUpper.includes("EXPORT")) {
    return ACTION_CONFIG.DOWNLOAD_ZIP;
  }

  return {
    label: action || "Khác",
    icon: <IconPlus className="size-3.5" />,
    badge: "text-gray-700 border-gray-200 bg-gray-50",
  };
}
    ...
    <div className="flex flex-col gap-3">
                  ...
                  >
                    <td className="py-3 px-4">
                      <Badge
                        variant="outline"
                        className={cn(
                          "flex items-center gap-1.5 w-fit text-xs px-2 py-0.5",
                          cfg.badge,
                        )}
                      >
                        {cfg.icon}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>
            {entries.length} bản ghi · Trang {page}/{totalPages}
          </span>
