...
const STATUS_CONFIG = {
  pending:    { label: "Chưa xử lý", cls: "text-yellow-700 border-yellow-200 bg-yellow-50 dark:bg-yellow-900/20 dark:text-yellow-300 dark:border-yellow-700" },
  generating: { label: "Đang tạo",   cls: "text-blue-700 border-blue-200 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-700" },
  ready:      { label: "Sẵn sàng",  cls: "text-green-700 border-green-200 bg-green-50 dark:bg-green-900/20 dark:text-green-300 dark:border-green-700" },
  failed:     { label: "Lỗi",        cls: "text-red-700 border-red-200 bg-red-50 dark:bg-red-900/20 dark:text-red-300 dark:border-red-700" },
};
...
const TYPE_CLS: Record<SmartReport["type"], string> = {
  daily:     "text-blue-700 border-blue-200 bg-blue-50 dark:bg-blue-950/30 dark:text-blue-400",
  weekly:    "text-purple-700 border-purple-200 bg-purple-50 dark:bg-purple-950/30 dark:text-purple-400",
  monthly:   "text-orange-700 border-orange-200 bg-orange-50 dark:bg-orange-950/30 dark:text-orange-400",
  quarterly: "text-indigo-700 border-indigo-200 bg-indigo-50 dark:bg-indigo-950/30 dark:text-indigo-400",
  custom:    "text-gray-700 border-gray-200 bg-gray-50 dark:bg-gray-950/30 dark:text-gray-400",
  incident:  "text-red-700 border-red-200 bg-red-50 dark:bg-red-950/30 dark:text-red-400",
};

/** Hàng danh sách báo cáo – compact, layout thay đổi theo trạng thái */
export function SmartReportRow({ report, query, canManage = false, onDelete, onClick }: Props) {
  ...
  const handleDownload = (fmt: "pdf" | "xlsx") => {
    window.open(getDownloadUrl(report.id, fmt), "_blank");
  };
  ...
  return (
    ...
    >
      ...
      <Badge
        variant="outline"
        className={cn("shrink-0 text-[10px] px-1.5 py-0.5 font-medium w-12 sm:w-16 justify-center", TYPE_CLS[report.type])}
      >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          <span className="text-[11px] text-muted-foreground">{formatPeriod()}</span>
          ...
          {isReady && report.summary_json && (
            <>
              <span className="sm:hidden text-[11px] text-muted-foreground">•</span>
      ...
      {/* ── Trạng thái ───────────────────────────────── */}
      <Badge variant="outline" className={cn("shrink-0 text-[10px] px-1.5 py-0.5 hidden xs:inline-flex", statusCfg.cls)}>
        {statusCfg.label}
      </Badge>
          ...
          <>
            {report.files_json.pdf && (
              <Tooltip>
                ...
                <TooltipContent>Tải PDF ({formatFileSize(report.files_json.pdf.sizeMB)})</TooltipContent>
              </Tooltip>
            )}
            {report.files_json.xlsx && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-7 w-7"
                    onClick={(e) => { e.stopPropagation(); handleDownload("xlsx"); }}>
                    <IconFileTypeXls className="h-4 w-4 text-green-600" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>Tải Excel ({formatFileSize(report.files_json.xlsx.sizeMB)})</TooltipContent>
              </Tooltip>
            )}
          </>
        )}
