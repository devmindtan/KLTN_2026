...
-- MV 2: Theo NGÀY trong tuần — scope: 7 ngày gần nhất (rolling window, không reset mỗi tuần)
...
SELECT
  ...
  COUNT(*)::INT                         AS sample_count
FROM camera_detections
WHERE created_at >= DATE_TRUNC('day', NOW() + INTERVAL '7 hours') - INTERVAL '1 hour' - INTERVAL '7 days'
...
-- MV 3: Theo TUẦN ISO — scope: 4 tuần ISO hoàn chỉnh gần nhất (rolling window)
...
SELECT
  ...
  COUNT(*)::INT                         AS sample_count
FROM camera_detections
WHERE created_at >= DATE_TRUNC('week', NOW() + INTERVAL '7 hours') - INTERVAL '1 hour' - INTERVAL '4 weeks'
  AND created_at <  DATE_TRUNC('week', NOW() + INTERVAL '7 hours') - INTERVAL '1 hour'
  AND EXTRACT(HOUR FROM (created_at + INTERVAL '7 hours')) BETWEEN 6 AND 23
GROUP BY dimension_value, camera_id;
...
-- MV 4: Theo THÁNG trong năm — scope: 12 tháng hoàn chỉnh gần nhất (rolling window)
...
SELECT
  ...
  COUNT(*)::INT                         AS sample_count
FROM camera_detections
WHERE created_at >= DATE_TRUNC('month', NOW() + INTERVAL '7 hours') - INTERVAL '1 hour' - INTERVAL '12 months'
  AND created_at <  DATE_TRUNC('month', NOW() + INTERVAL '7 hours') - INTERVAL '1 hour'
  AND EXTRACT(HOUR FROM (created_at + INTERVAL '7 hours')) BETWEEN 6 AND 23
GROUP BY dimension_value, camera_id;
